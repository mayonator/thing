 ...
 * Bot invite link
 */
const invite_link = `https://discord.com/api/oauth2/authorize?client_id=718282860385009724&permissions=8&scope=bot`;
...
client.on('ready', () => {
    ...
    client.user.setActivity(`v!help | ${Number(userAmount).toLocaleString()} users | https://i.bonk.ml`);
    setInterval(() => {
        userAmount = 0;
        client.guilds.cache.forEach((guild) => {
            guild.members.cache.forEach(() => {
...
client.on('message', msg => {
    ...
    if (msg.content.startsWith(`${prefix}verify `) && !isDM()) {
        ...
        if (isDM()) {
            ...
            msg.channel.send(`${icon.x} No username found\nThe command should be formatted like so: \`${prefix}verify foobball\`, where "foobball" is your username.`);
        }
        ...
        msg.channel.send(
            embed.create(
                `Help`,
                `PURPLE`,
                `A list of all commands for the HyVerify bot.`, [{
                    "title": "`Verification`",
        ...
        msg.channel.send(`${icon.x} The GitHub is now private.`);
        ...
        msg.channel.send(`If you want to support me or the bot donate to my Ko-Fi page: https://ko-fi.com/foobball`);
...
client.on("guildCreate", guild => {
    ...
    guild.owner.send(
        embed.create(
            'Thanks for adding HyVerify bot',
            'PURPLE',
